...
@app.cell
def decoder_weights_16(SAE_REPO_16, mo, range_bytes, safetensors_header):
    try:
...
@app.cell
def controls(feature_catalog, mo):
    ...
    mo.vstack([
        mo.md("## Figure 4D | Multi-dimensionality decoder neighborhood (improved)\n\nReproduction of Figure 4D showing the feature neighborhood around **F6716** (nucleophilic elbow catalytic motif) across SAE dimensionalities **2^13**, **2^14**, and **2^16**, projected with UMAP and shown as different sized dots.\n\nPaper (p.12): *\"Feature neighborhood around F6716 in the SAE decoder space for multiple feature dimensionalities: 2^13, 2^14, and 2^16, shown as different sized dots.\"*\n\nPaper (p.12-13): *\"The neighborhood is biologically coherent, containing other flexible catalytic loops and phosphorylation motifs. Features from SAEs trained on the same representations with larger or smaller feature dimensions show substantial overlap between their directions... one feature in the 2^13 space corresponds to ten in the 2^16 space.\"*"),
        mo.hstack([anchor_feature, show_cluster_labels, structure_selector], gap=1),
        mo.md("_2^13 (84 MB) and 2^14 (168 MB) decoders are loaded automatically. 2^16 (671 MB) is loaded above if available._"),
    ])
    return anchor_feature, show_cluster_labels, structure_selector


@app.cell
def multi_dim_neighborhood(
    CODEBOOK_13,
    CODEBOOK_14,
    CODEBOOK_16,
    FEATURE_API,
    ...
    neighborhood_source = {
        ...
        "anchor_label": _anchor_info.get("label", ""),
        "counts": _counts,
    ...
    mo.md(
        f"**Neighborhood computed.** {_counts[CODEBOOK_13]} features from 2^13, "
        f"{_counts[CODEBOOK_14]} from 2^14, {_counts[CODEBOOK_16]} from 2^16. "
        f"Total: {len(neighbor_rows)} features at cosine >= 0.1. "
        f"Projection: {_projection_name}."
    )
...
def panel_d_scatter(
    ...
    _dim_sizes = {CODEBOOK_13: 22, CODEBOOK_14: 14, CODEBOOK_16: 8}
    _dim_colors = {CODEBOOK_13: "#1f77b4", CODEBOOK_14: "#ff7f0e", CODEBOOK_16: "#2ca02c"}
    _dim_names = {CODEBOOK_13: "2^13 (8,192)", CODEBOOK_14: "2^14 (16,384)", CODEBOOK_16: "2^16 (65,536)"}

    _visible = list(neighbor_rows)
    _non_anchor = [r for r in _visible if not r["is_anchor"]]
    _anchor_rows = [r for r in _visible if r["is_anchor"]]

    _fig = go.Figure()

    for _dim in [CODEBOOK_16, CODEBOOK_14, CODEBOOK_13]:
        _dim_rows = [r for r in _non_anchor if r["sae_dim"] == _dim]
        if not _dim_rows:
            continue
        _colors = []
        for r in _dim_rows:
            if r["sae_dim"] == CODEBOOK_14:
                _colors.append(_category_colors.get(r["category"], "#64748b"))
            else:
                _colors.append(_dim_colors[_dim])
        _fig.add_trace(go.Scattergl(
            x=[r["x"] for r in _dim_rows], y=[r["y"] for r in _dim_rows],
            mode="markers", name=_dim_names[_dim],
            marker={"size": _dim_sizes[_dim], "color": _colors, "opacity": 0.82, "line": {"width": 0.5, "color": "white"}},
            customdata=[[r["key"], r["feature_id"], r["sae_dim"], r["category"], r["label"], r["cosine"]] for r in _dim_rows],
            hovertemplate="<b>%{customdata[1]}</b><br>SAE: %{customdata[2]}<br>Category: %{customdata[3]}<br>Cosine: %{customdata[5]:.3f}<br>%{customdata[4]}<extra></extra>",
        ))

    if _anchor_rows:
        _a = _anchor_rows[0]
        _fig.add_trace(go.Scatter(
            x=[_a["x"]], y=[_a["y"]], mode="markers+text", name="anchor F6716",
            text=[_a["feature_id"]], textposition="top center",
            marker={"symbol": "star", "size": 26, "color": "#111827", "line": {"width": 1, "color": "white"}},
            customdata=[[_a["key"], _a["feature_id"], _a["sae_dim"], _a["category"], _a["label"], _a["cosine"]]],
            hovertemplate="<b>%{customdata[1]}</b> anchor<br>SAE: %{customdata[2]}<br>Cosine: %{customdata[5]:.3f}<extra></extra>",
        ))

    if show_cluster_labels.value and _non_anchor:
        _cats = sorted(set(r["category"] for r in _non_anchor if r["sae_dim"] == CODEBOOK_14))
        for _cat in _cats:
            _cat_rows = [r for r in _non_anchor if r["category"] == _cat and r["sae_dim"] == CODEBOOK_14]
            if _cat_rows:
                _fig.add_annotation(
                    x=sum(r["x"] for r in _cat_rows) / len(_cat_rows),
                    y=sum(r["y"] for r in _cat_rows) / len(_cat_rows),
                    text=_cat, showarrow=False,
                    font={"size": 10, "color": _category_colors.get(_cat, "#64748b")},
                )

    _counts = neighborhood_source["counts"]
    _fig.update_layout(
        height=650, template="plotly_white", dragmode="select", clickmode="event+select",
        title=f"F{int(anchor_feature.value[1:])} decoder neighborhood | cosine >= 0.1 | {_counts.get(CODEBOOK_13, 0)} + {_counts.get(CODEBOOK_14, 0)} + {_counts.get(CODEBOOK_16, 0)} features",
        xaxis={"visible": False, "showgrid": False, "zeroline": False},
        yaxis={"visible": False, "showgrid": False, "zeroline": False},
        legend={"orientation": "h", "y": -0.05},
        margin={"l": 15, "r": 15, "t": 55, "b": 40},
    )

    neighbor_plot = mo.ui.plotly(_fig, config={"displaylogo": False, "scrollZoom": True}, label="Click a decoder feature")
    panel_d_scatter = mo.vstack([
        mo.md("### Interactive decoder directions\nEach dot is an SAE feature, not a protein. **Larger dots = smaller SAE** (2^13), **medium dots = 2^14**, **small dots = 2^16** - matching the paper's 'different sized dots' convention. Color encodes feature category (2^14 only; 2^13/2^16 use dim-specific colors since no API metadata). Click any dot to load its metadata, residue activations, and 3D structure below."),
        neighbor_plot,
        mo.md(f"Projection: `{neighborhood_source['projection']}`\n\nUMAP params: `{neighborhood_source['umap_params']}`\n\nPaper ref: `{neighborhood_source['paper_ref']}`"),
    ])
    panel_d_scatter
...
@app.cell
def biological_coherence(CODEBOOK_14, mo, neighbor_rows, table_html):
...
def paper_claims(
    ...
    _claims = [
        ...
        },
        {
            ...
            "verified": "paper only",
            "detail": "Requires protein-level activation analysis (Figure S30). Not reproduced in this notebook - see detail panel for individual protein activations.",
        },
        {
